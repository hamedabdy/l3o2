 ...
 */
function setUserLocation(latitude, longitude, range, artist,onlypij) {
    ...
    getConcerts(latitude, longitude, range, artist, onlypij);
}
...
function geoCodeAddress(address, range, artist) {
    var address = document.getElementById('address').value,
        range = parseFloat($('#range').slider('value')),
        artist = document.getElementById('artist').value,
        onlypij = document.getElementById('tofetch').checked;
    ...
    if(address != '') {
        geocoder.geocode({ "address" : address }, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                setUserLocation(latitude, longitude, range, artist, onlypij);
 ...
 * AJAX call to server
 */
function getConcerts(lat, lng, range, artist, onlypij) {
    var _responseJSON;
    
    $.ajax({
        type : 'GET',
        url : '/concert?lat='+lat+'&long='+lng+'&range='+range+'&artist=' + artist + '&onlypij=' + onlypij,
