...
app.get('/addpijconcert', ensureAuthenticated, function(req, res){
  res.render('addpijconcert', { user: req.user });
});
...
function encryptPass(pass, fn){
    ...
    var hash = passwordHash.generate(pass)
        if(hash){ 
		return fn(null, hash); }
		else{
			return fn(null, null);
